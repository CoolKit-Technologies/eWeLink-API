(function () {
  ...
  function main(config) {
    // Carl 新增
    var name = config.name && typeof config.name === "string"
  ...
  function cover() {
    ...
    if (windowWidth < 640) {
      var bgc = "background-color: #fff;";
    }
    if (windowWidth >= 640) {
      var bgc = "background:url('./media/img/bg.jpg')center center / cover;";
    }
    return (
      '<section class="cover show" style="' +
      bgc +
      '">' +
      '<div class="cover-main"><!--cover--></div>' +
      '<div class="mask"></div>' +
  ...
  ********************************************** */
  ...
  function renderName(vm) {
    ...
    if (isPrimitive(vm.config.name)) {
      ...
      var match = Object.keys(name).filter(function (key) {
