...
登录 [易微联开发者平台](https://dev.ewelink.cc)，提交认证资料，等待审核完成，请注意免费的 APPID 存在限制，详细介绍请查阅「接口调用规范与 APPID 权限说明（重要）】」章节和「收费标准」栏目。
...
- 通过易微联开发者平台登录完成认证的开发者使用的「免费 APPID」请求 OAuth2.0 相关接口，单个区域所有接口的总请求次数限制 5 万次/月，超出限额，接口返回 HTTP 状态码为 403 错误（不是返回的参数），次月额度刷新可继续使用，如需立即解除限制需要付费升级为企业版，请邮件联系 bd@coolkit.cn。
